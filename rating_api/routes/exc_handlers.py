...
from rating_api.exceptions import (AlreadyExists, CommentTooLong,
                                   ForbiddenAction, ForbiddenSymbol,
                                   ObjectNotFound, TooManyCommentRequests,
                                   TooManyCommentsToLecturer, UpdateError,
                                   WrongMark)
...
async def forbidden_action_handler(
    req: starlette.requests.Request, exc: ForbiddenAction
):
    return JSONResponse(
        content=StatusResponseModel(
            status="Error", message=exc.eng, ru=exc.ru
...
async def comment_too_long_handler(
    req: starlette.requests.Request, exc: CommentTooLong
):
    return JSONResponse(
        content=StatusResponseModel(
            status="Error", message=exc.eng, ru=exc.ru
...
async def forbidden_symbol_handler(
    req: starlette.requests.Request, exc: ForbiddenSymbol
):
    return JSONResponse(
        content=StatusResponseModel(
            status="Error", message=exc.eng, ru=exc.ru
